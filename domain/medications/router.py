...
@router.delete("/api/medications", summary="약 정보 삭제")
async def delete_medications(record: MedicationDelete, db: Session = Depends(get_db)):
    """
    약의 정보를 삭제하는 엔드포인트입니다.
    **pill_id** : int
    """
    try:
        return delete_pills(db, record)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))